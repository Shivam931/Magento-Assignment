...
class Index extends \Magento\Framework\App\Action\Action
{
    ...
    {
        if ($this->getRequest()->getParam('source_redirect_slug'))
        {
            // Stripe calls this URL twice. Once with a source_redirect_slug parameter, and another with a redirect_status parameter.
            // Allowing both to run causes a duplicate order email, so we ignore the first one.
            return;
        }
        ...
        {
            $this->helper->setProcessingState($order);
            $this->helper->saveOrder($order);
        }
